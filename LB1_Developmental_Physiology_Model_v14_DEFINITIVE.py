"""
LB1 DEVELOPMENTAL PHYSIOLOGY MODEL — VERSION 14 DEFINITIVE THESIS MODEL
...
Version 14 separates two levels of biology:
...
Therefore Version 14 does NOT invent a precise "Flores population mean."
...
def print_v14_registry():
    print("=" * 92)
    print("LB1 DEVELOPMENTAL PHYSIOLOGY MODEL — VERSION 14 DEFINITIVE THESIS MODEL")
    print("=" * 92)
    print("PRESPECIFIED PARAMETER REGISTRY")
    print("-" * 92)
    print("Flores linear scale              : 0.64–0.80  [fossil-constrained sensitivity prior]")
    print("Brain allometry exponent         : 1.60–2.20  [comparative-allometry sensitivity]")
    print("Normalized IGF perturbation      : 0.00–1.00  [dimensionless sensitivity parameter]")
    print("Iodine burden                    : 0.00–1.00  [normalized latent deficiency index]")
    print("Intergenerational transmission   : 0.10–0.29  [literature-anchored sensitivity]")
    print(f"Simulations per experiment       : {N:,}")
    print(f"Random seed                      : {SEED}")
    print("-" * 92)
    print("Interpretation: compatibility/sensitivity model; not a reconstruction of LB1 causation.")
    print("=" * 92)
...
print()
print("=" * 92)
print("VERSION 14 MODEL COMPARISON")
print("=" * 92)
...
summary.to_csv(
    os.path.join(
        BASE,
        "LB1_v14_population_baseline_summary.csv"
    ),
    index=False
)
...
assumptions.to_csv(
    os.path.join(
        BASE,
        "LB1_v14_population_baseline_assumptions.csv"
    ),
    index=False
)
...
plt.title(
    "Version 14: Effect of Flores Population Baseline"
)
plt.xticks(
    rotation=40,
    ha="right"
...
plt.savefig(
    os.path.join(
        BASE,
        "LB1_v14_population_baseline_comparison.png"
    ),
    dpi=300
)
plt.close()
...
plt.savefig(
    os.path.join(
        BASE,
        "LB1_v14_baseline_to_LB1_decomposition.png"
    ),
    dpi=300
)
plt.close()
...
plt.title(
    "Version 14: Population Baseline and Developmental Phenotype Space"
)
plt.legend()
plt.tight_layout()
plt.savefig(
...
print()
print("=" * 92)
print("VERSION 14 INTERPRETATION")
print("=" * 92)

print("""
Version 14 explicitly separates population-level small body size from
...
print("VERSION 14 COMPLETE")


# ============================================================
# VERSION 14 THESIS FREEZE
# ============================================================
# The Version 14 architecture above is retained unchanged as the biological
# core. Version 14 adds transparency, structural-score robustness, frozen
...
V13_REGISTRY.to_csv(os.path.join(BASE,"LB1_v14_parameter_registry.csv"),index=False)
...
V13_CORR.to_csv(os.path.join(BASE,"LB1_v14_correlation_adjusted_robustness.csv"),index=False)

print("\n"+"="*92)
print("VERSION 14 CORRELATION-ADJUSTED ROBUSTNESS")
print("="*92)
...
V13_SENS.to_csv(os.path.join(BASE,"LB1_v14_global_module_sensitivity.csv"),index=False)

print("\n"+"="*92)
print("VERSION 14 GLOBAL MODULE-LEVEL SENSITIVITY")
print("="*92)
...
# Re-use the frozen developmental pathways from Version 14, changing only baseline width.
...
V13_ROBUST.to_csv(os.path.join(BASE,"LB1_v14_prior_width_robustness.csv"),index=False)
print("\n"+"="*92)
print("VERSION 14 FROZEN PRIOR-WIDTH ROBUSTNESS")
print("="*92)
...
V13_COMPONENTS.to_csv(os.path.join(BASE,"LB1_v14_best_component_errors.csv"),index=False)
...
plt.title("Version 14 Global Module-Level Sensitivity")
plt.tight_layout(); plt.savefig(os.path.join(BASE,"LB1_v14_global_module_sensitivity.png"),dpi=300); plt.close()

plt.figure(figsize=(9,6))
plt.bar(V13_COMPONENTS["Endpoint"],V13_COMPONENTS["Squared_Error"])
plt.ylabel("Squared standardized error")
plt.title("Version 14 Best Full-Model Error by Endpoint")
plt.xticks(rotation=25,ha="right")
plt.tight_layout(); plt.savefig(os.path.join(BASE,"LB1_v14_best_component_errors.png"),dpi=300); plt.close()

print("\n"+"="*92)
print("VERSION 14 THESIS FREEZE RULES")
print("="*92)
print("""
...
""")
print("VERSION 14 DEFINITIVE THESIS MODEL COMPLETE")
